...
//    public static Queue<BwPlayer> PLAYER_WAITING_QUEUE = new ConcurrentLinkedQueue<>();
    ...
    public Bw_general(NoodleLegs plugin) {
        this.plugin = plugin;
    }
...
//                player.sendMessage("Please select a range for your map " + ChatColor.YELLOW + "/bw pos1" + ChatColor.GREEN + " and " + ChatColor.YELLOW + "/bw pos2");
    ...
    private boolean doDelete( String[] args, Player player){
        if (args.length < 2) {
...
//        }
    ...
    private void doList( String[] args, Player player) {
        if (args.length > 1) {
...
//        EDIT = true;
        if (bwMode == BW_Mode.EDIT) {
            player.sendMessage(ChatColor.RED + "You cant change to edit mode when its already edit mode");
        }else{
            bwMode = BW_Mode.EDIT;
            player.sendMessage(ChatColor.GREEN + "The game has been set to editing mode");
            player.sendMessage(ChatColor.YELLOW + "Editing mode! You can now break blocks!!!!");
        }
...
//        EDIT = false;
        if (bwMode == BW_Mode.SAVE){
            player.sendMessage(ChatColor.RED + "You cant change to save mode when its already save mode");
        }else{
            bwMode = BW_Mode.SAVE;
            player.sendMessage(ChatColor.GREEN + "The game has been set to save mode");
            player.sendMessage(ChatColor.YELLOW + "You are now no longer in edit mode, so you can not change anything. ");
        }
...
//                setting points on map
                ...
                if (firstArg.equalsIgnoreCase("setlobbyWorld")){
                    player.sendMessage(ChatColor.RED + "This command is unavailable at this time. ");
